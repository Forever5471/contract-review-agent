...
description: 基于合同画像执行风险审查，完成规则选择、脚本模式规则执行、指令模式规则占位执行、轻量 RAG 证据检索和风险项生成。适用于合同理解完成后，需要输出可溯源风险列表、规则命中过程和人工确认标记的场景。
---
...
根据合同类型、字段和全文执行合同审查。该 Skill 代表“风险审查”这个业务能力，规则引擎、RAG 检索和脚本执行都只是它内部的工具。
...
- 脚本模式：使用确定性 Python 判断，适合金额阈值、字段存在性、关键词模式等稳定规则。
- 指令模式：优先使用 GLM 指令执行器，要求模型只返回 JSON；未配置或调用失败时回退启发式判断和 RAG 证据。
